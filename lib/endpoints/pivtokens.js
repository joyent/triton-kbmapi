 ...
 * Parameters:
 *
 * @param {String} guid the PIV Token GUID. Required.
...
function updateRecoveryTokens(req, res, next) {
    ...
    // 3. Flag the staged recovery configuration if not already done.
    // 4. Expire the previously staged recovery configuration if not the same.
    ...
    if (req.params.zpool_recovery.active) {
        requests = requests.concat({
            ...
            filter: util.format(
                ...
                req.params.zpool_recovery.active)
        }, {
            bucket: mod_recovery_token.bucket().name,
            operation: 'update',
            fields: {
    ...
    if (req.params.recovery_token && req.params.zpool_recovery.staged) {
        requests.push({
            ...
            filter: util.format(
                ...
                '(!(staged=*))(!(expired=*)))',
                req.params.zpool_recovery.staged, req.params.recovery_token)
        });
    ...
    req.app.moray.batch(requests, function batchCb(batchErr, _batchMeta) {
        if (batchErr) {
            next(batchErr);
            return;
        }
