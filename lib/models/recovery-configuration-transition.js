...
// TODO: Shouldn't a transition be automatically completed when created w/o
// targets at all? Note this needs to happen at fsm-transition to make sure
// we also set the right value for recovery configuration state.
...
const BUCKET = {
    ...
    schema: {
        index: {
            ...
            // If we use a type 'date' here, using a filter like
            // '(!(finished=*))' will not work.
            started: { type: 'string' },
...
const CREATE_SCHEMA = {
    required: {
        ...
        forced: validate.bool,
        standalone: validate.bool,
...
function createRecoveryConfigurationTransition(opts, cb) {
    ...
    if (typeof (opts.params.forced) === 'undefined') {
        opts.params.forced = false;
    }

    if (typeof (opts.params.standalone) === 'undefined') {
        opts.params.standalone = false;
    }
...
module.exports = {
    ...
    RecoveryConfigurationTransition: RecoveryConfigurationTransition,
