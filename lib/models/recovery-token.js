...
RecoveryToken.prototype.serialize = function serialize() {
    ...
    var obj = {
        token: this.params.token ?
            new Buffer(this.params.token).toString('base64') : undefined,
        ...
        funcs: [
            ...
            function batchQuery(ctx, next) {
                ...
                opts.moray.batch(requests,
                    ...
                    opts.moray.log.debug({
                        requests: requests,
                        batchErr: batchErr,
                        batchMeta: batchMeta
                    }, 'createRecoveryToken: batchQuery batch');
                    if (batchErr) {
                        cb(batchErr);
                        return;
                    }
...
function setActiveRecoveryToken(opts, cb) {
    ...
    assert.string(opts.pivtoken_guid, 'opts.pivtoken_guid');
    assert.func(cb, 'cb');
    // Get the "one and only" staged recovery token so we are sure that it is
    // the recovery token we are activating and, at the same time, that we're
    // expiring the previously active one, if any.
    mod_moray.listObjs({
        filter: util.format(
            '(&(pivtoken=%s)(staged=*)(!(activated=*))(!(expired=*))' +
            '(recovery_configuration=%s))',
            opts.pivtoken_guid, opts.active_recovery_config_uuid),
        bucket: BUCKET,
        log: opts.log,
        moray: opts.moray,
        model: RecoveryToken
    }, function listCb(listErr, listRes) {
        if (listErr) {
            cb(listErr);
            return;
        }
        ...
        if (listRes.length !== 1) {
            cb(new VError('Unexpected number of recovery tokens to activate: ' +
                listRes.length));
            return;
        }
        ...
        const requests = [{
            ...
            filter: util.format('(uuid=%s)', tk.key())
        }];
        ...
        opts.moray.batch(requests, function batchCb(batchErr, batchMeta) {
            opts.log.debug({
                ...
                batchErr: batchErr,
                batchMeta: batchMeta
            }, 'setActiveRecoveryToken batch');

            if (batchErr) {
                opts.log.error({
                    error: batchErr,
                    activated_recovery_config_uuid:
                        opts.active_recovery_config_uuid,
                    recovery_token_uuid: tk.key()
                }, 'Error setting active recovery token');
            }
...
function setStagedRecoveryToken(opts, cb) {
    ...
    const requests = [{
        ...
        filter: util.format(
            '(&(!(uuid=%s))(staged=*)(!(activated=*))' +
            '(!(expired=*))(pivtoken=%s))',
            opts.recovery_token_uuid, opts.pivtoken_guid)
    }, {
        bucket: BUCKET.name,
        operation: 'update',
        fields: {
