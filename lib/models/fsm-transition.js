 ...
 *
 *   When we cancel a WIP transition, the attribute `aborted` for the
 *   associated `RecoveryConfigurationTransition` record is set to `true`.
 *   Which may eventually cause an early finish of such transition if needed.
 *   See `cancelTransition` below.
 *
...
const RECOVERY_CONFIGURATION_STATIC_STATES = {
    ...
    'staged': {
        validTransitions: ['stage', 'unstage', 'activate']
    },
    'active': {
        validTransitions: ['activate', 'expire', 'deactivate']
    },
    'expired': {
            ...
            mod_recovery_token.ls({
                ...
                opts.moray.batch(requests,
                    ...
                    opts.log.debug({
                        ...
                        batchErr: batchErr,
                        batchMeta: batchMeta
                    }, 'transition: removeTransitions batch');
                    if (batchErr) {
                        next(batchErr);
                        return;
                    }
