 ...
 *
 * TODO: Remember to filter the already transitioned recovery tokens when
 * processing recovery cfg trs.
 *
...
function transition(opts, cb) {
    ...
    vasync.pipeline({arg: context, funcs: [
        ...
        function checkValidTransition(ctx, next) {
            ...
            if (action !== 'cancel' && validTrs.indexOf(action) === -1) {
                next(new errors.InvalidParamsError(
                    ...
                    action, getRecCfgState(ctx.recoveryConfiguration))]
                ));
                return;
            }
            next();
         ...
         * What it's not possible is to have more than one of these transitions
        ...
        /*
         * Both 'reactivate' and 'expire' will result into straight
